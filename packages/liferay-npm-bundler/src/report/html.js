...
export function htmlDump(report) {
	...
	const title = '<h1>Report of liferay-npm-bundler execution</h1>';

	const summary = htmlTable([
		htmlRow(`<td>Executed at:</td><td>${_executionDate}</td>`),
		...
		.join(' ')}
			...
			rows.push(
				htmlRow(`
					${cells.map(cell => `<td>${cell}</td>`).join(' ')}
					${sourceCell}
					<td class="${msg0.level}">${msg0.level}</td>
			...
			for (let i = 1; i < msgs.length; i++) {
				...
				rows.push(
					htmlRow(`
						...
						${sourceCell}
						<td class="${msgs[i].level}">${msgs[i].level}</td>
